<?php
global $curauth, $wpdb, $count_love_fans, $leaf;
$break_page = 20;
$users = $wpdb->get_results('select * from wp_user_item_taxonomy, wp_users where wp_user_item_taxonomy.item_type = "user" and wp_user_item_taxonomy.user_id = wp_users.ID and wp_user_item_taxonomy.item_id = '.$curauth->ID.' order by wp_user_item_taxonomy.id desc limit '.($leaf-1)*$break_page.', '.$break_page);
$max_page = ceil($count_love_fans/$break_page);
?>
...
foreach($users as $key => $user_item){
  ...
  // 未知昵称
  $nn = $user_item->display_name;
  if(empty($nn)){
    $nn = $user_item->user_nicename;
    $nn = substr_replace($nn, '****', 3, count($nn)-5);
  }
?>
<div class="col-md-6 no_pama">
	<div class="collection-wrap">
		...
		<div class="collection-text">
			...
			<a href="<?php echo get_author_posts_url($user_item->ID);?>" class="item_name"><?php echo $nn; ?></a></h3>
