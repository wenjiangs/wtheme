...
$count_posts = count_user_posts($current_user->ID, 'topic');
$max_page = ceil($count_posts/$break_page);

//文章状态统计
$count_pending = $wpdb->get_var('select count(*) from wp_posts where post_type = "topic" and post_status = "pending" and post_author = '.$current_user->ID);
$count_draft = $wpdb->get_var('select count(*) from wp_posts where post_type = "topic" and post_status = "draft" and post_author = '.$current_user->ID);
$count_trash = $wpdb->get_var('select count(*) from wp_posts where post_type = "topic" and post_status = "trash" and post_author = '.$current_user->ID);
$count_private = $wpdb->get_var('select count(*) from wp_posts where post_type = "topic" and post_status = "private" and post_author = '.$current_user->ID);
...
<div class="wt-container" style="overflow:auto;">
	<h1 class="wt-setting-tit">我的话题 <a href="/new-topic" class="btn btn-success btn-xs">写文章</a></h1>
	...
	<table class="article_table">
		...
		//更新回复数
		$count_reply = $wpdb->get_var('select count(*) from wp_comments where comment_post_ID = '.$posts_item->ID);
		if(!($count_reply == $posts_item->comment_count)){
			$wpdb->query('update wp_posts set comment_count = '.$count_reply.' where ID = '.$posts_item->ID);
		}
		$posts_item->comment_count = $count_reply;
		?>
		<tr>
			<td><a href="<?php echo get_permalink($posts_item->ID); ?>"><?php echo $posts_item->post_title; ?></a></td>
			<td width="180"><?php echo date('Y-m-d', strtotime($posts_item->post_date)); ?> (<?php echo get_post_meta($posts_item->ID, 'views', true); ?>/<?php echo $posts_item->comment_count; ?>)</td>
